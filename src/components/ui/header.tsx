...
export default function Header() {
  ...
  return (
    <header className="z-[999]  flex items-center justify-center py-20 sm:py-36">
      ...
      <div
        className="fixed top-0 h-[4.5rem] w-full rounded-none border border-white/40 bg-white/30 shadow-lg shadow-black/[0.03]  backdrop-blur-[0.5rem] duration-500 animate-in slide-in-from-top dark:border-black/40 dark:bg-gray-950/30 sm:top-6 sm:h-[3.25rem] sm:w-[36rem] sm:rounded-full"
      >
          ...
          <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-5 gap-y-1 text-gray-500 sm:w-[initial] sm:flex-nowrap md:gap-5">
            {links.map(link => (
              <li className="flex h-3/4 items-center" key={link.hash}>
                <Link
                  className={clsx(
                    'relative flex w-full items-center justify-center p-3 transition hover:text-gray-950 dark:hover:text-gray-50',
                ...
                >
                  ...
                  {link.name === activeSection && (
                    <motion.span
                      className="absolute inset-0 -z-10 m-1 rounded-full bg-gray-300/50 dark:bg-gray-700/50"
